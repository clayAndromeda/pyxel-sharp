...
// Shared between JumpGame (desktop) and JumpGame.Web (browser-wasm); the
// resource path differs per host, so the constructor takes it as a parameter.
using PyxelSharp;
...
class App
{
    ...
    public App(string resourcePath)
    {
        Pyxel.Init(160, 120, title: "Pyxel Jump");
        Pyxel.Load(resourcePath);
